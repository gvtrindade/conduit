          ...
          <div className="p-4.5 flex gap-4 items-start relative z-10">
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-1.5">
                <span className="font-mono text-[9px] font-bold tracking-[0.08em] uppercase px-2 py-0.5 rounded-sm border text-amber border-amber bg-amber/10">
                  CAPTAIN
                </span>
                <span className="font-mono text-[9px] font-bold tracking-[0.08em] uppercase px-2 py-0.5 rounded-sm border text-green border-green bg-green/10">
                  VERIFIED
                </span>
              </div>
            </div>
          ...
          <div className="bg-panel border-2 border-border-custom rounded-xl overflow-hidden">
            ...
            <div className="p-3.5">
              <div className="flex items-center justify-between mb-2.5">
                <span
                  className="font-heading text-[40px] font-bold text-amber leading-none"
                  style={{ textShadow: "0 0 30px rgba(217,140,69,0.35)" }}
                >
                  0{profile.level}
                </span>
                <div className="text-right">
                  <span className="font-tight text-lg font-bold text-cream uppercase tracking-[0.04em] block">
                    CAPTAIN
                  </span>
                  <span className="font-mono text-[9px] text-sand tracking-[0.08em] uppercase">
              ...
              <ProgressBar
                value={profile.xp}
                max={profile.xpNext}
                color="linear-gradient(90deg, var(--amber), rgba(217,140,69,0.5))"
                height="h-2"
              />
              <div className="flex justify-between mt-1.5 font-mono text-[8px] tracking-[0.08em] uppercase">
                ...
                <span className="text-amber">
                  ▲ {(profile.xpNext - profile.xp).toLocaleString()} TO
                  COMMANDER
                </span>
              </div>
              ...
              <div className="flex justify-between mt-3 pt-3 border-t border-border-custom">
                {[
                  ...
                  <div
                    key={rank.num}
                    className={`text-center ${rank.dim ? "opacity-35" : rank.active ? "" : "opacity-50"}`}
                  >
                    <div
                      className={`font-mono text-[8px] ${rank.active ? "text-amber text-[10px] font-bold" : "text-sand"}`}
                    >
                      {rank.num}
                    </div>
                    {rank.active && (
                      <div
                        className="w-1.5 h-1.5 rounded-full bg-amber mx-auto mt-1"
                        style={{ boxShadow: "0 0 8px var(--amber)" }}
                      />
                    )}
                    <div
                      className={`font-mono text-[8px] mt-0.5 ${rank.active ? "text-amber" : "text-sand"}`}
                    >
                      {rank.name}
                    </div>
                  </div>
            ...
            {[
              {
                icon: "🎯",
                name: "Budget Sniper",
                desc: "Stayed under budget 3 months straight.",
                progress: 100,
                color: "green",
                status: "UNLOCKED",
              },
              {
                icon: "📦",
                name: "Bulk Hauler",
                desc: "Single receipt exceeding 200 kCr.",
                progress: 100,
                color: "amber",
                status: "UNLOCKED",
              },
              {
                icon: "🔭",
                name: "Deep Scanner",
                desc: "Track 200 unique items.",
                progress: Math.min(
                  Math.round((profile.itemsTracked / 200) * 100),
                  100,
                ),
                color: "blue",
                status: `${profile.itemsTracked} / 200`,
              },
              {
                icon: "🛸",
                name: "Fleet Admiral",
                desc: "Reach Commander rank or above.",
                progress: 0,
                color: "sand",
                status: "LOCKED",
                locked: true,
              },
              {
                icon: "📉",
                name: "Frugal Ops",
                desc: "Log a -10% variance in a month.",
                progress: Math.min(
                  Math.round(
                    (Math.abs(parseFloat(profile.variance)) / 10) * 100,
                  ),
                  100,
                ),
                color: "green",
                status: `${profile.variance} / -10%`,
              },
              {
                icon: "⚡",
                name: "Crisis Pilot",
                desc: "Override a receipt anomaly manually.",
                progress: 0,
                color: "sand",
                status: "LOCKED",
                locked: true,
              ...
              <div
                key={ach.name}
                className={`bg-panel border border-border-custom rounded-xl p-3 flex flex-col gap-1.5 ${ach.locked ? "opacity-45" : ""}`}
              >
                <div className="flex items-center gap-2">
                  <div
                    className="w-[34px] h-[34px] rounded-lg flex items-center justify-center text-base flex-shrink-0"
                    style={{ background: `var(--${ach.color})12` }}
                  >
                    {ach.icon}
                  </div>
                  <div className="font-mono text-[10px] font-bold tracking-[0.06em] uppercase text-cream leading-tight">
                    {ach.name}
                  </div>
                </div>
                <div className="text-[11px] text-sand leading-relaxed">
                  {ach.desc}
                </div>
                <div className="font-mono text-[8px] text-sand mb-0.5">
                  {ach.status}
                </div>
                <ProgressBar
                  value={ach.progress}
        ...
        <div className="px-5 pt-4 pb-3.5">
          ...
          <div className="bg-panel border-2 border-border-custom rounded-xl overflow-hidden">
            {[
              ...
              <div
                key={pref.name}
                className="flex items-center justify-between px-3.5 py-3 border-b border-border-custom last:border-b-0"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-hull border border-border-custom flex items-center justify-center text-sm">
                    {pref.icon}
                  </div>
                  <div>
                    <div className="font-mono text-[11px] font-bold tracking-[0.06em] uppercase text-cream">
                      {pref.name}
                    </div>
                    <div className="font-mono text-[9px] text-sand mt-0.5 tracking-wider">
                      {pref.sub}
                    </div>
                  </div>
                </div>
                <span
                  className={`font-mono text-[10px] font-bold tracking-[0.06em] uppercase px-2 py-0.5 rounded border ${
                    pref.valColor === "green"
        ...
        <div className="px-5 pb-4">
          ...
          <div className="bg-red/[0.06] border border-red/30 rounded-xl overflow-hidden">
            {[
              ...
              {
                icon: "🔄",
                name: "RESET_BUDGET_CYCLE",
                color: "amber",
                action: null,
              },
              {
                icon: "💀",
                name: "PURGE_ALL_MANIFESTS",
                color: "red",
                action: null,
              },
              ...(session
