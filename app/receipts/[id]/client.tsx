...
import {
  ...
  deleteReceiptItem,
  updateReceiptItem,
} from "@/lib/receipt-item-mutations";
...
export default function ReceiptDetailClient({
  ...
  const { data: rawReceipts, isLoading: receiptLoading } = useQuery(
    RECEIPT_DETAIL_QUERY,
    [id],
  );
  ...
  const { data: rawMerchants } = useQuery(
    "SELECT id, name, emoji FROM merchants ORDER BY name",
  );
  const { data: rawCatalogItems } = useQuery(
    "SELECT id, name, unit, last_price FROM items ORDER BY name",
  );
  ...
  const catalogItems = useMemo(() => {
    ...
    return rawCatalogItems as Array<{
      id: string;
      name: string;
      unit: string;
  ...
  const receipt = useMemo(() => {
    ...
    const items = ((rawItems as unknown as DbReceiptItemRow[]) || []).map(
      mapDbReceiptItemToReceiptItem,
    );
  ...
  return (
    <div className="relative flex-1 flex flex-col">
      ...
      <TopNav
        ...
        backLabel="RCPT_LOG"
        title={
          isEditing ? `EDIT RECEIPT #${receipt.id}` : `RECEIPT #${receipt.id}`
        }
      ...
      {isEditing ? (
        <div className="flex-1 overflow-y-auto scrollbar-none pb-7 px-5 pt-4">
          <ReceiptEditForm
            ...
            onCreateMerchant={async (name, emoji) => {
              return await createMerchant(powerSync, {
                name,
                emoji,
                user_id: userId,
                created_at: null,
        ...
        <div className="flex-1 overflow-y-auto scrollbar-none pb-7">
            ...
            <div className="bg-panel border-2 border-border-custom rounded-2xl overflow-hidden relative">
              ...
              <div className="p-4 relative z-10">
                <div className="flex items-start justify-between mb-2.5">
                  <div className="w-11 h-11 rounded-xl bg-hull border border-border-custom flex items-center justify-center text-xl">
                    🏪
                  </div>
                  <Badge
                    variant={
                ...
                <div className="mt-2.5">
                  <div className="font-tight text-[22px] font-bold text-cream uppercase tracking-[0.04em] leading-tight">
                    {receipt.merchant.split("_").map((w, i) => (
                      <span key={i}>
                        {w}
                        {i < receipt.merchant.split("_").length - 1 ? " " : ""}
                      </span>
                    ))}
                  </div>
          ...
          <div className="px-5 py-4 border-b border-border-custom flex items-end justify-between">
            <div>
              <div className="font-mono text-[9px] font-bold tracking-[0.16em] uppercase text-sand mb-1">
                // TOTAL_OUTLAY //
              </div>
              <div>
                <span className="font-heading text-[44px] font-bold text-cream leading-none tracking-tight">
                  {receipt.total.toFixed(2)}
                </span>
                <span className="font-mono text-base text-sand ml-1.5">
                  kCr
                </span>
              </div>
            </div>
          ...
          {receipt.items.length > 0 && (
            <div className="px-5">
              <SectionLabel>
                // LINE_ITEMS // {receipt.itemCount} UNITS //
              </SectionLabel>
              <div className="bg-panel border-2 border-border-custom rounded-xl overflow-hidden">
                <div className="grid grid-cols-[1fr_52px_52px_62px] px-3.5 py-[7px] bg-hull border-b border-border-custom gap-2">
                  <span className="font-mono text-[8px] font-bold tracking-[0.12em] uppercase text-sand">
                    ITEM
                  </span>
                  <span className="font-mono text-[8px] font-bold tracking-[0.12em] uppercase text-sand text-right">
                    QTY
                  </span>
                  <span className="font-mono text-[8px] font-bold tracking-[0.12em] uppercase text-sand text-right">
                    UNIT
                ...
                {receipt.items.map((item, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`grid grid-cols-[1fr_52px_52px_62px] px-3.5 py-2.5 border-b border-border-custom last:border-b-0 gap-2 items-start hover:bg-panel2 transition-colors ${isFlagged ? "bg-red/[0.06] border-l-2 border-l-red" : isDiscount ? "bg-green/[0.05]" : ""}`}
                    >
                      <div>
                        <div className="text-xs font-medium text-cream leading-snug">
                          {item.name}
                        </div>
                        <div className="flex gap-1 mt-0.5">
                          {item.tags.map((tag) => (
                            <span
                              key={tag}
                              className={`font-mono text-[8px] uppercase tracking-wider inline-block px-1 py-px rounded-sm border ${tagStyles[tag] || "text-sand bg-sand/10 border-sand/25"}`}
                            >
                              {tag}
                            </span>
                          ))}
                      ...
                      <span className="font-heading text-xs font-bold text-sand text-right pt-px">
                        {item.qty}
                      </span>
                      <span className="font-heading text-xs font-bold text-sand text-right pt-px">
                      ...
                      <span
                        className={`font-heading text-xs font-bold text-right pt-px ${isFlagged ? "text-red" : isDiscount ? "text-green" : "text-cream"}`}
                      >
          ...
          <div className="px-5 pt-4">
            ...
            <div className="grid grid-cols-2 gap-2 mb-2.5">
              {[
                ...
                <button
                  key={action.label}
                  onClick={() =>
                    showToast(action.icon, action.label.toUpperCase())
                  }
            ...
            >
              ...
              <span className="font-mono text-[11px] font-bold tracking-[0.1em] uppercase text-red">
                [ PURGE_RECEIPT_LOG ]
              </span>
            </button>
          </div>
      ...
      <ModalOverlay show={showDelete} onClose={() => setShowDelete(false)}>
        <div className="bg-hull px-5 py-3.5 border-b border-border-custom flex items-center gap-2.5">
          ...
          <span className="font-mono text-[11px] font-bold tracking-[0.1em] uppercase text-red flex-1">
            PURGE_CONFIRM
          </span>
        </div>
        <div className="p-5">
          <p className="text-sm text-sand leading-relaxed mb-4">
            You are about to permanently delete receipt{" "}
            <strong className="text-cream">#{receipt.id}</strong>. This action
            cannot be undone.
          </p>
