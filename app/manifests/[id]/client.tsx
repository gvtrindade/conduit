...
import { updateManifest, activateManifest, completeManifest, archiveManifest, removeManifestItem, toggleManifestItemChecked, deleteManifest } from '@/lib/manifest-mutations';
...
export default function ManifestDetailClient({ id }: { id: string }) {
  ...
  const canAddItems = mft?.status === 'draft';
  ...
  const STATUS_TRANSITIONS: Record<string, { label: string; bg: string; border: string; shadow: string; textColor: string } | null> = {
    draft: { label: 'ACTIVATE_MANIFEST', bg: 'bg-green', border: 'border-[#1B8452]', shadow: '0 0 20px rgba(49,196,132,0.3)', textColor: 'text-hull' },
    active: { label: 'MARK_COMPLETE', bg: 'bg-amber', border: 'border-[#C07830]', shadow: '0 0 20px rgba(217,140,69,0.3)', textColor: 'text-hull' },
    done: { label: 'ARCHIVE', bg: 'bg-sand', border: 'border-[#8B7D6B]', shadow: '0 0 20px rgba(191,175,155,0.3)', textColor: 'text-hull' },
    archived: null,
  };
  ...
  const handleToggleChecked = useCallback(async (itemId: string, checked: boolean) => {
    ...
    <div className="relative flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-none pb-6">
        <div className="px-5 py-3 border-b border-border-custom">
          ...
          <div className="mb-3">
            <div className="font-mono text-[9px] text-sand tracking-[0.1em] uppercase mb-1.5">MANIFEST_TYPE:</div>
            {editable ? (
              <div className="flex gap-1.5 flex-wrap">
                {MANIFEST_TYPES.map((type) => (
                  <button
                    ...
                    className={`font-mono text-[9px] font-bold tracking-[0.1em] uppercase px-2.5 py-1 rounded-full border-[1.5px] cursor-pointer transition-all ${
                      mft.type === type
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex items-center gap-1.5 flex-1">
              <div className="flex-1 h-2 bg-hull border border-border-custom rounded-sm overflow-hidden">
                <div className="h-full bg-gradient-to-r from-green to-green/50 rounded-sm" style={{ width: `${checkedPercentage}%` }} />
              </div>
              <span className="font-mono text-[9px] text-amber">{confidenceDisplay}</span>
            </div>
          </div>
          )}
          {transitionConfig && (
            <button
              ...
              style={{ boxShadow: canTransition ? `inset 0 -3px 0 rgba(0,0,0,0.4), ${transitionConfig.shadow}` : 'inset 0 -3px 0 rgba(0,0,0,0.2)' }}
            >
        ...
        <div className="flex items-center justify-between px-5 pt-4 pb-2">
          ...
          {canAddItems && (
            <button
              ...
              className="font-mono text-[9px] font-bold tracking-[0.1em] uppercase text-blue bg-blue/8 border-[1.5px] border-blue/35 rounded-md px-2.5 py-1 cursor-pointer hover:border-blue hover:bg-blue/14 transition-all ml-2 flex items-center gap-1"
            >
        ...
        <div className="px-5 flex flex-col gap-2">
          {mft.items.map((item, i) => (
            <div key={item.id || i} className={`bg-panel border border-border-custom rounded-lg px-3.5 py-3 flex items-center gap-3 ${item.checked ? 'opacity-65' : ''} ${item.unknown ? 'border-amber/30' : ''}`}>
              {mft.status !== 'draft' && (
                <>
                  {canToggleChecked ? (
                    <button
                    ...
                    <div className={`w-[22px] h-[22px] flex-shrink-0 border-2 rounded-sm bg-hull flex items-center justify-center font-mono text-sm font-bold text-green ${item.checked ? 'border-green' : item.unknown ? 'border-amber' : 'border-border-custom'}`}>
                      {item.checked && '✕'}
                    </div>
                  )}
                </>
      ...
      <ModalOverlay show={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)}>
        ...
        <ModalBody>
          <div className="text-center mb-6">
            ...
            <div className="font-mono text-xs font-bold tracking-[0.1em] uppercase text-cream mb-2">DELETE_MANIFEST</div>
            <div className="font-mono text-[10px] text-sand">This will remove all items. This action cannot be undone.</div>
          </div>
