...
export default function ItemDetailClient({ id }: { id: string }) {
  ...
  const recentReceipts = [
    { merchant: 'SECTOR_7_WHOLE_FOODS', date: '2024.10.14', id: 'WF-2024-1014-7734', price: 0.89, tag: 'SALE' },
    { merchant: 'TRADER_JOE_STN', date: '2024.10.05', id: 'TJ-2024-1005-2291', price: 0.92, tag: 'NORMAL' },
    { merchant: 'COSTCO_SECTOR', date: '2024.09.22', id: 'CS-2024-0922-8847', price: 0.78, tag: 'BEST' },
    { merchant: 'TRADER_JOE_STN', date: '2024.09.08', id: 'TJ-2024-0908-1143', price: 0.95, tag: '▲ HIGH' },
    { merchant: 'SECTOR_7_WHOLE_FOODS', date: '2024.08.25', id: 'WF-2024-0825-5512', price: 0.85, tag: 'NORMAL' },
  ];

  const points = item.priceHistory.map((p, i) => {
    const x = 30 + (i / (item.priceHistory.length - 1)) * 250;
    const minP = Math.min(...item.priceHistory.map(h => h.price));
    const maxP = Math.max(...item.priceHistory.map(h => h.price));
  ...
  const fillD = `${pathD} L ${points[points.length - 1].x} 140 L ${points[0].x} 140 Z`;
    ...
    <div className="relative flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-none pb-6">
        ...
        <div className="px-5 grid grid-cols-2 gap-2.5">
          ...
          <DataField label="FREQ_SOURCE" value={item.freqSource} />
        ...
        <div className="px-5 pt-4">
          ...
          <div className="bg-panel border-2 border-border-custom rounded-xl overflow-hidden">
            {recentReceipts.map((rcpt, i) => (
              <Link key={i} href="/receipts/093" className="no-underline">
                <div className="flex items-center gap-3 px-3.5 py-2.5 border-b border-border-custom last:border-b-0 cursor-pointer hover:bg-panel2 transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="font-mono text-[9px] text-sand tracking-wider">{rcpt.date} · #{rcpt.id}</div>
                  </div>
